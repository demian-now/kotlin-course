        ...
        for (i in input) {
            if (i.first == TypeOfElement.Number)
                ...
                if (stack.isEmpty() || stack.last().second == "(")
                    ...
                    while (stack.isNotEmpty() && (stack.last().second != "(" || (getPriority(stack.last().second) < getPriority(
                            i.second)))
                    ) {
        ...
        while (input.size > 1) {
            ...
            for (i in 0..input.size - 3) {
                if (input[i].first == TypeOfElement.Operator
                    ...
                    val value2 =
                        if ("pie".contains(input[i + 2].second)) if (input[i + 2].second == "pi") pi else e else input[i + 2].second.toDouble()
                    input[i] = Pair(
